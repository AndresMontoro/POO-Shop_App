...
Tarjeta::Tarjeta(const Numero& n, Usuario& u, const Fecha& f): numero_{n}, titular_(&u), caducidad_{f}, activa_{true}
{
    ...
    if (f < hoy)
    {
        ...
        throw c;   
    }else if (!tarjetas_.insert(n).second)  
    {
...
void Tarjeta::anula_titular()
{
    ...
    //activa_ = false;     // el enunciado dice que la desactive pero Pedro en clase me dijo que no tenia sentido desactivarla y además daba error
...
std::ostream& operator <<(std::ostream& os, const Tarjeta& t) noexcept
{
    ...
    // Cambiamos minusculas a mayusculas en el nombre
